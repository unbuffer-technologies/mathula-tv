...
DIRECT_DUB_SCHEMA_VERSION = (
    "mathula-direct-azure-dub-v47-word-anchored-islands-v13.18.56"
)
...
FINAL_PRODUCT_CONDUCTOR_VERSION = (
    "mathula-final-product-conductor-v1-v13.18.18"
)
SOURCE_WORD_ALIGNMENT_VERSION = (
    "mathula-source-word-speaker-alignment-v1-v13.18.19"
)
SOURCE_PAUSE_ALIGNMENT_VERSION = (
    "mathula-source-pause-alignment-v5-island-floor-calibration-v13.18.45"
)
...
SEMANTIC_FIT_BATCH_STRATEGY_VERSION = (
    "mathula-semantic-fit-global-batch-v21-version-scoped-budget-v13.18.58"
)
...
PERFORMANCE_PLAN_VERSION = (
    "mathula-performance-plan-v9-adaptive-island-drift-v13.18.57"
)
WORD_ANCHORED_ISLAND_CONTRACT_VERSION = (
    "mathula-word-anchored-island-contract-v2-v13.18.57"
)
...
class DirectDubOptions:
    ...
    boundary_max_time_stretch_ratio: float = 1.08
    final_product_max_time_stretch_ratio: float = 2.0
    max_cross_speaker_boundary_shift_ms: int = 500
    source_word_alignment_tolerance_ms: int = 250
        ...
        adjusted.append(
            replace(
                ...
                source_speech_start_ms=(
                    int(block_words[0]["start_ms"]) if block_words else None
                ),
                source_speech_end_ms=(
                    int(block_words[-1]["end_ms"]) if block_words else None
                ),
            )
        )
        if anchors:
            artifact.append(
                {
                    ...
                    "source_speech_start_ms": int(block_words[0]["start_ms"]),
                    "source_speech_end_ms": int(block_words[-1]["end_ms"]),
                    "anchors": anchors,
                }
            )
...
class DirectAzureDubRenderer:
    ...
    def _semantic_fit_payload(
        ...
        *,
        ...
        payload.update(
            {
                ...
                "performance_plan": (
                    {
                        ...
                        "contract": (
                            ...
                            "island_id must appear exactly once in ordered "
                            "island_deliveries. Beats may own one or more "
                            "adjacent islands."
                        ),
                ...
                "source_phrase_cadence": plan_phrase_cadence(
                    block_id=block.block_id,
        ...
        # The most recent Azure observation is the control signal. Earlier
        # versions selected the shortest historical attempt, which could keep
        # asking GPT to contract after the delivery had already underfilled.
        measured_frontier = next(
            (
                value
                for value in reversed(duration_evidence)
                if isinstance(value, Mapping)
        ...
        if measured_frontier is not None:
            ...
            if required_reduction_ms and frontier_characters:
                # Azure duration, not GPT's estimate, owns this feedback.
                # A small headroom factor prevents another just-over-the-line
                # paraphrase while remaining an advisory linguistic signal.
            ...
            if isinstance(island_measurements, list) and island_measurements:
                payload["performance_island_alignment"] = (
                    cls._performance_island_alignment_summary(
                        island_measurements
                    )
                )
        ...
        *,
    ...
    ) -> dict[int, tuple[SpeechBlock, dict[str, Any], list[dict[str, Any]]]]:
            ...
            if (
                ...
                (
                    ...
                    self._measure_performance_islands(
                        ...
                        pitch_percent=state["pitch"],
                        volume_percent=state["volume"],
                        tolerance_ms=options.semantic_fit_tolerance_ms,
                    )
                )
        ...
        for index, block in enumerate(blocks):
            ...
            minimum, preferred, maximum = self._semantic_fit_target_band(
                block, options.semantic_fit_tolerance_ms
            )
        ...
        def strategy_attempt_limit(state: Mapping[str, Any]) -> int:
            # The CLI/configured semantic-fit attempt budget is authoritative.
            # Performance regions already generate only one measured revision
            # per round, so a second hidden four-attempt cap prematurely stops
            # the closed-loop controller before it can converge.
            return options.semantic_fit_max_attempts

        def strategy_attempts_used(state: Mapping[str, Any]) -> int:
            """Return attempts consumed by the active strategy only.

            Historical checkpoints contain absolute attempt numbers spanning
            several semantic-fit strategies.  A persisted arithmetic baseline
            is not reliable after an upgrade or interrupted resume.  Newer
            attempts already carry both strategy_version and strategy_attempt,
            so make those tags authoritative.  Changing strategy version then
            gives every unresolved unit a fresh configured budget while a
            rerun of the same strategy resumes exactly where it stopped.
            """
            return max(
                (
        ...
        while portfolio_supported:
            ...
            for index, remaining in eligible:
                ...
                # Legacy semantic-fit retains its portfolio sizes unchanged.
                per_round_limit = (
                    1
                    if performance_region
                    else (3 if len(eligible) == 1 else 6)
                )
                ...
                payload["portfolio_contract"] = {
                    ...
                    "performance_plan_candidate_count": (
                        1 if performance_region else None
                    ),
    ...
    def _force_final_product_fit(
        ...
        *,
        ...
        order, or timeline boundaries. It accepts a visibly audited cadence
        compromise so a paid unattended job completes instead of waiting for a
        panel decision. FFmpeg's chained atempo stages support ratios above 2x;
        the configured ceiling is a quality target, while the exact measured
        ratio is the technical completion requirement.
        """
    ...
    def _measure_performance_islands(
        ...
        *,
        ...
        for index, (source_island, text) in enumerate(
            ...
            maximum_ms = target_ms + island_tolerance_ms
            request = AzureTTSRequest(
            ...
            measurements.append(
                {
                    ...
                    "status": "fit" if fits else "timing_miss",
                    "speaker_rate_percent": base_rate_percent,
                }
            )
        ...
        for measurement, normalized in zip(
            ...
            measurement["raw_isolated_duration_fit"] = measurement["status"] == "fit"
            ...
            try:
                if options.timing_mode in {
                    ...
                    selected_block, fitted, variant_audit = (
                        semantic_batch_results[block_index]
                    )
                else:
