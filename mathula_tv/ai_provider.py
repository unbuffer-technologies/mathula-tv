...
SEMANTIC_FIT_PROMPT_VERSION = "gpt-semantic-fit-zu-v11-binding-island-feedback-v13.18.57"
SEMANTIC_FIT_REVIEW_PROMPT_VERSION = (
    "gpt-semantic-fit-review-zu-v4-v13.18.54"
)
...
The speaker rate is locked and audio may not be stretched. Use mandatory_island_pause_ms and maximum_plain_speech_ms as hard acoustic accounting: the spoken wording must leave room for protected source-speech island pauses. Use the source_speech_islands as a delivery blueprint. Prefer compact isiZulu morphology, concise clause fusion and context-recoverable agreement over clipped words, apostrophe-heavy spelling or deleted meaning.
...
When performance_plan is present, ignore STT member-block boundaries as linguistic boundaries and plan the complete region as a natural target-language performance. Return performance_beats in semantic order. A beat is a meaning unit, not necessarily a sentence. Each beat must own one or more adjacent source islands through island_deliveries. Across all beats, every supplied source island_id must appear exactly once and in source order. Each island delivery contains the exact target words associated with that source mouth-active interval. Joining every island delivery spoken_text in source order with one space must reproduce the unit spoken_text, allowing punctuation and spacing differences only. Meaning may move across internal STT boundaries but never across the immutable speaker lane, interaction boundary or region endpoint. Whole-region Azure duration and immutable endpoints are the primary hard lip-sync gate. Individually synthesized islands carry artificial phrase-boundary overhead, so use the normalized performance_island_alignment rather than raw isolated durations. Its maximum_allowed_cumulative_drift_ms is binding when supplied: if a boundary exceeds it, redistribute wording between adjacent islands in the indicated direction while keeping the whole-region duration inside the binding band. Never change tempo or stretch audio.

Return candidate_count alternatives per block. In performance-plan feedback rounds candidate_count is one: revise the latest measured delivery in exactly the requested acoustic_correction direction instead of producing parallel guesses from stale evidence. When more than one alternative is requested, order them from fullest likely fit to shortest faithful fit; they must use materially different grammatical constructions and must not duplicate rejected_spoken_texts or one another. candidate_id must be candidate_01, candidate_02, and so on in order. estimated_plain_speech_ms excludes inserted SSML pauses. compression_strategy must state the real grammatical change used. Keep faithful_translation anchored to the approved meaning; spoken_text is the delivery overlay and tts_text may differ only for approved pronunciation handling.

Return strict JSON matching the supplied schema. The application will Azure-synthesize every candidate at the locked rate and independently review timing-fit candidates before selection."""
    ...
    def fit_semantic_portfolios(
        ...
        for item in requested:
            ...
            if isinstance(performance_plan, Mapping):
                ...
                # Beat/island objects repeat IDs and three text fields, so the
                # former 1,200-token allowance per candidate was too small.
                # This is an output ceiling, not a request to be verbose.
                per_candidate_budget = min(
                    5_000,
                    1_800 + island_count * 220,
                )
            else:
                per_candidate_budget = 1_200
        ...
        return self.complete_structured(
            StructuredAIRequest(
                ...
                prompt_version=f"{SEMANTIC_FIT_PROMPT_VERSION}-portfolio-v1",
                ...
                max_output_tokens=min(
                    64_000,
                    max(output_token_budget, candidate_total * 1_800)
                    + GPT_SEMANTIC_FIT_REASONING_RESERVE_TOKENS,
                ),
                normalizer=lambda value: (
            ...
            candidates.append(
                {
                    ...
                    "candidate": _normalise_protected_turn_repair_result(
                        request_item,
                        list(placeholders_by_unit.get(unit_id, [])),
                        dict(candidate),
                    ),
        ...
        units.append(
            {
                ...
                "performance_beats": [
                    dict(value)
                    for value in _as_list(raw.get("performance_beats"))
                    if isinstance(value, Mapping)
                ],
