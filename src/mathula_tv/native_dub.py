...
# --- Bracketing-candidate smoothing -----------------------------------------------
# Real user proposal, 2026-09-10: "I'm think[ing] of a potential smoothing call for
# candidates that undershoot and overshoot, by using the candidates to create [a]
# middle candidate." Confirmed real, repeated shape this session (native_phrase_0024,
# native_phrase_0003): the natural translation overshoots its window, and the ONLY
# available safe cut (dropping one whole clause) overshoots the CORRECTION instead,
# landing as an undershoot -- a lumpy, coarse jump with no rung in between. Rather than
# build finer-grained sub-clause/word editing (the exact machinery this session already
# tried twice -- Won et al. iterative shorten, word-level Gaussian redistribution -- and
# reverted for cost/reliability reasons), this asks the model to interpolate between two
# already-real, already-measured, already-literal-safe texts it can see directly, with
# their real durations as concrete anchors -- a narrowly-scoped, well-grounded ask, not
# an open-ended "make it shorter" retry loop.

SENTENCE_MIDDLE_CANDIDATE_PROMPT_VERSION = "native-sentence-middle-candidate-v2-syllable-target"

SENTENCE_MIDDLE_CANDIDATE_SYSTEM_PROMPT = r"""For each unit, you are given the TRUE English source_text,
a "fuller_text" isiZulu rendering that is measured as slightly too LONG for its real broadcast window,
and a "shorter_text" isiZulu rendering (a real, already-safe alternative) that is measured as slightly
too SHORT -- both real spoken-duration measurements are given in milliseconds, alongside the real
target window. Write ONE new isiZulu retelling, middle_text, that lands BETWEEN fuller_text and
shorter_text in real spoken length -- genuinely closer to the target_window_ms than either extreme,
not simply a copy of one of them.

You are also given each text's own syllable count (fuller_syllables, shorter_syllables) and the
window's own target_syllables (with min_syllables/max_syllables as the acceptable range around it) --
isiZulu is a syllable-timed language, so this is a more direct, language-native planning unit than raw
milliseconds: aim to write middle_text at roughly target_syllables, somewhere between fuller_syllables
and shorter_syllables, rather than reasoning about duration in the abstract. This is a planning aid,
not the final judge -- your real spoken output will be measured for real afterward regardless.

Prefer reaching the middle through TIGHTER, more economical phrasing of fuller_text's own full content
(a leaner connective, no restating an already-established referent, no padding) -- this is the SAME
concision judgment already used elsewhere in this pipeline, and it lets you keep every fact fuller_text
carries while still landing shorter. Only if pure economy genuinely cannot close enough of the gap,
you may drop the SAME content shorter_text already dropped -- but if you do, state whatever remains
slightly more fully/naturally than shorter_text did, since shorter_text is now known to have cut too
much. Every fact, name, number, date, and negation present in shorter_text must also be present in
middle_text -- middle_text must never contain LESS real content than shorter_text already safely
carries, only ever the same or more, phrased more economically than fuller_text.

Return every requested unit_id exactly once. No tools or web search. Return JSON only."""


def _sentence_middle_candidate_schema(expected_unit_ids: Sequence[str]) -> dict[str, Any]:
    requested_ids = [str(value) for value in expected_unit_ids]
    if len(set(requested_ids)) != len(requested_ids):
        raise ValueError("Sentence middle candidate schema received duplicate expected unit IDs")
    return {
        "type": "object",
        "additionalProperties": False,
        "required": ["units"],
        "properties": {
            "units": {
                ...
                "items": {
                    "type": "object",
                    "additionalProperties": False,
                    "required": ["unit_id", "middle_text"],
                    "properties": {
                        "unit_id": {"type": "string", "enum": requested_ids},
                        "middle_text": {"type": "string", "minLength": 1},
                    },
                },
            },
        },
...
def _request_middle_candidate_batch(
    *, provider: FoundryGrokProvider, items: Sequence[Mapping[str, Any]],
) -> tuple[dict[str, str], dict[str, int]]:
    """items: [{"unit_id", "source_text", "fuller_text", "fuller_measured_ms",
    "shorter_text", "shorter_measured_ms", "target_window_ms", "fuller_syllables",
    "shorter_syllables", "target_syllables", "min_syllables", "max_syllables"}, ...].
    Returns {unit_id: middle_text}. Identity mismatch on unit_id is fatal (same
    convention as every other candidate-translate-family function).
    """
    expected_ids = [str(item["unit_id"]) for item in items]
    response = provider.complete_json(
        operation="native_sentence_middle_candidate_batch",
        system_prompt=SENTENCE_MIDDLE_CANDIDATE_SYSTEM_PROMPT,
        payload={
            "units": [
                {
                    "unit_id": str(item["unit_id"]), "source_text": str(item["source_text"]),
                    "fuller_text": str(item["fuller_text"]),
                    "fuller_measured_ms": int(item["fuller_measured_ms"]),
                    "fuller_syllables": int(item["fuller_syllables"]),
                    "shorter_text": str(item["shorter_text"]),
                    "shorter_measured_ms": int(item["shorter_measured_ms"]),
                    "shorter_syllables": int(item["shorter_syllables"]),
                    "target_window_ms": int(item["target_window_ms"]),
                    "target_syllables": int(item["target_syllables"]),
                    "min_syllables": int(item["min_syllables"]),
                    "max_syllables": int(item["max_syllables"]),
                }
    ...
    for item in returned:
        ...
        if unit_id not in expected_set:
            unknown_ids.append(unit_id)
            continue
        by_id[unit_id] = str(item.get("middle_text") or "").strip()
    ...
    if duplicate_ids or unknown_ids or missing_ids:
        ...
        if duplicate_ids:
            details.append("duplicate=" + ",".join(duplicate_ids))
        raise ValueError("Sentence middle candidate batch identity mismatch: " + "; ".join(details))
...
def _smooth_bracketing_candidates(
    *,
    provider: FoundryGrokProvider,
    groups: Sequence[Mapping[str, Any]],
    winners: Mapping[str, Mapping[str, Any]],
    ...
    """For a block whose winner still does not fit after _trim_by_fact_priority
    -- and where that mechanism's own OWN ladder produced no candidate landing
    inside the window because the lightest safe cut overshoots the correction
    into an undershoot -- ask for one new candidate interpolated between the
    two real, already-measured texts that bracket the target (see the module
    comment above for the real motivating cases and why this differs from the
    two previously-reverted redistribution mechanisms).

    Trigger, precisely: the current winner's own direction is "compress" (an
    overshoot) AND the block's lightest shortened_candidate, once freely
    re-measured, is a genuine "expand" (an undershoot) -- i.e. the two real
    texts actually bracket the window with nothing already landing inside it.
    A block whose lightest cut is STILL an overshoot (needs a deeper ladder
    rung, already tried by _trim_by_fact_priority) is not a bracket case and
    is skipped here -- this mechanism only ever interpolates, never searches.

    Never worse than the winner passed in: promoted only if the middle
    candidate's real measured rank beats the current winner's, exactly like
    every other mechanism in this file. Always sets requires_review when
    promoted, so a human always sees a smoothed result before it airs.
    """
    groups_by_id = {str(g["group_id"]): g for g in groups}
    usage_totals = {"input_tokens": 0, "output_tokens": 0, "attempts": 0}
    updated_winners = dict(winners)
    ...
    for gid, winner in updated_winners.items():
        ...
        if group is None:
            continue
        shortened = group.get("shortened_candidates") or []
        if not shortened:
            continue
        geometry = geometries.get(gid)
        if geometry is None:
            continue
        baseline_direction = _timing_recast_direction(
            int(winner.get("measured_ms") or 0), geometry["source_window_ms"],
            max_speed_percent=DEFAULT_MAX_NATURAL_SPEED_PERCENT,
    ...
    probe_measured = _measure_temporal_batch(
        tts=tts, groups=[groups_by_id[gid] for gid in bracket_candidates],
        candidates_by_group={gid: [candidate] for gid, candidate in bracket_candidates.items()},
        geometries={gid: geometries[gid] for gid in bracket_candidates},
        voice_assignments=voice_assignments, output_root=measurement_audio_root,
        round_number=11, force=force, workers=candidate_pool_tts_workers,
    )

    triggered: dict[str, dict[str, Any]] = {}
    for gid, candidate in bracket_candidates.items():
        measured = probe_measured.get(gid) or []
        if not measured:
            continue
        geometry = geometries[gid]
        probe = measured[0]
        probe_direction = _timing_recast_direction(
            int(probe["measured_ms"]), geometry["source_window_ms"],
            max_speed_percent=DEFAULT_MAX_NATURAL_SPEED_PERCENT,
            mouth_close_early_tolerance_ms=geometry["mouth_close_early_tolerance_ms"],
            mouth_close_late_tolerance_ms=geometry["mouth_close_late_tolerance_ms"],
        )
        if probe_direction != "expand":
            continue  # not a genuine bracket -- still overshooting, or already fits
        triggered[gid] = {
            "shorter_text": candidate["spoken_text"], "shorter_measured_ms": int(probe["measured_ms"]),
        }
    ...
    _emit_progress(
        progress,
        f"[native candidate pool] {len(triggered)} block(s) bracket their window (overshoot vs. "
        "undershoot, nothing in between) -- requesting a middle candidate...",
    )

    items = []
    for gid, info in triggered.items():
        fuller_text = str(updated_winners[gid]["spoken_text"])
        window_ms = int(geometries[gid]["source_window_ms"])
        # Real user suggestion, 2026-09-10: give the model a syllable-based
        # target too, not just raw milliseconds -- isiZulu is syllable-timed,
        # so this is a more direct, language-native planning unit. Reuses the
        # exact same budgeting machinery _payload_window already feeds the
        # primary turn-block translation call.
        syllable_budget = _rolling_syllable_budget(
            {"start_ms": 0, "end_ms": window_ms},
            preferred_raw_speed_percent=preferred_raw_speed_percent,
            content_syllables_per_second=DEFAULT_ZULU_CONTENT_SYLLABLES_PER_SECOND,
            azure_utterance_overhead_ms=DEFAULT_AZURE_UTTERANCE_OVERHEAD_MS,
            tolerance_percent=DEFAULT_SYLLABLE_TOLERANCE_PERCENT,
        ...
        items.append({
            ...
            "fuller_text": fuller_text,
            "fuller_measured_ms": int(updated_winners[gid].get("measured_ms") or 0),
            "fuller_syllables": _estimate_zulu_syllables(fuller_text),
            "shorter_text": info["shorter_text"], "shorter_measured_ms": info["shorter_measured_ms"],
            "shorter_syllables": _estimate_zulu_syllables(info["shorter_text"]),
    ...
    try:
        middle_by_id, usage = _request_middle_candidate_batch(provider=provider, items=items)
    except Exception as exc:
        _emit_progress(progress, f"[native candidate pool] Middle-candidate request failed: {exc}")
        return updated_winners, usage_totals
    for key in usage_totals:
        usage_totals[key] += usage.get(key, 0)
    ...
    for gid in triggered:
        ...
        if missing:
            ...
            continue
        candidates_by_group[gid] = [{
            "candidate_id": f"{gid}__middle_candidate", "variant_id": "middle_candidate",
            "translator": "grok", "spoken_text": middle_text,
            "qa_penalty": 0, "qa_record": None, "requires_review": False,
        }]
    ...
    raw_measured = _measure_temporal_batch(
        ...
        geometries={gid: geometries[gid] for gid in measured_group_ids},
        voice_assignments=voice_assignments, output_root=measurement_audio_root,
        round_number=12, force=force, workers=candidate_pool_tts_workers,
    )
    ...
    for gid in measured_group_ids:
        ...
        if candidate_rank < baseline_rank:
            ...
            _emit_progress(
                progress,
                f"[native candidate pool] {gid} improved via bracket smoothing: "
                f"{baseline_required_speed:.1f}% -> {candidate_required_speed:.1f}%: {candidate['spoken_text']!r}.",
            )
        else:
            _emit_progress(
                progress,
                f"[native candidate pool] Middle candidate for {gid} did not improve on the original "
                f"({baseline_required_speed:.1f}% baseline vs {candidate_required_speed:.1f}% for the "
...
def build_candidate_pool(
    *,
    ...
    skip_bracket_smoothing: bool = False,
    stt_backend: AzureFastTranscriptionBackend | None = None,
    progress: Callable[[str], None] | None = None,
) -> dict[str, Any]:
    ...
    input_hash = _hash_payload({
        ...
        "skip_bracket_smoothing": bool(skip_bracket_smoothing),
        "sentence_middle_candidate_prompt_version": SENTENCE_MIDDLE_CANDIDATE_PROMPT_VERSION,
    ...
    smoothing_usage = {"input_tokens": 0, "output_tokens": 0, "attempts": 0}
    if not skip_bracket_smoothing:
        winners, smoothing_usage = _smooth_bracketing_candidates(
            ...
            preferred_raw_speed_percent=int(preferred_raw_speed_percent), progress=progress,
        )
        _merge_updated_winners(winners)
    else:
        _emit_progress(progress, "[native candidate pool] Bracket smoothing skipped (diagnostic mode).")
    ...
    for usage in (
        ...
        smoothing_usage, last_resort_usage,
    ):
        for key in usage_totals:
            usage_totals[key] += int(usage.get(key) or 0)
