...
ZULU SOCIAL MEDIA VOCABULARY -- applies to EVERY unit, regardless of speaker_seriousness_mode:
Mathula TV's audience is a social-media audience for ALL content, not only general_news -- a
cross-examination clip or a political speech is still consumed as a social-media video by the same
real isiZulu-speaking viewers, so their own everyday vocabulary is the right target register
everywhere, not a special case for one kind of content. Widen the SUBSTITUTION license above beyond
modern political/social/institutional concept nouns to ANY ordinary word or short phrase (a verb, an
adjective, a common noun, a casual connective) where a code-switched or lightly nativized English/
Afrikaans-origin form is genuinely how real isiZulu-speaking social-media users actually say it,
even when a fully native isiZulu word exists and is not wrong. Confirmed real case: an overwhelming
majority of real isiZulu speakers on social media say "i-racism", not the formal "ukucwasa
ngokobuhlanga" -- for a word like this, the code-switched form is not merely an acceptable stylistic
option, it is the more accurate rendering of how this specific audience actually talks. Judge case
by case using your own knowledge of real contemporary isiZulu social-media/vernacular speech --
never invent a code-switch that isn't genuinely how people talk, and never force one where it reads
as unnatural or unclear. This license changes ONLY vocabulary and phrasing choice within
already-decided content -- it never touches which clauses survive (that is governed entirely by the
clause-ranking rules elsewhere in this prompt, including their own speaker_seriousness_mode gating),
and it NEVER weakens this prompt's own preservation rules for names, numbers, dates, negation,
attribution, or a claim/denial's own substance, all of which stay exactly as precise and unabridged
...
ZULU SOCIAL MEDIA VOCABULARY -- applies to EVERY window, regardless of speaker_seriousness_mode:
Mathula TV's audience is a social-media audience for ALL content, not only general_news -- a
cross-examination clip or a political speech is still consumed as a social-media video by the same
real isiZulu-speaking viewers, so their own everyday vocabulary is the right target register
everywhere, not a special case for one kind of content. Widen the SUBSTITUTION license above beyond
...
by case using your own knowledge of real contemporary isiZulu social-media/vernacular speech --
never invent a code-switch that isn't genuinely how people talk, and never force one where it reads
as unnatural or unclear. This license changes ONLY vocabulary and phrasing choice within an
already-decided retelling -- it is completely independent of, and never a substitute for, THE SECOND
EXCEPTION below (which governs whether a whole clause/list item may be DROPPED, not how a surviving
clause is WORDED, and stays gated by speaker_seriousness_mode exactly as written there), and it
NEVER weakens the HARD RULE below: names, numbers, dates, quotations, attributions, and a claim/
denial's own core proposition stay exactly as precise and unabridged regardless of register.
