...
PANEL_VERSION = "mathula.timeline-collision-panel.v4-no-dead-loop"
...
def _triplet_role_block(case: Mapping[str, Any], role: str) -> dict[str, Any]:
...
def _normalised_spoken_text(value: Any) -> str:
...
def save_timeline_collision_resolution(
    *,
    ...
    if strategy == "allow_overlap":
        ...
        text = _normalised_spoken_text(manual_text)
        if not text:
            raise ValueError("manual_text strategy requires non-empty text")
        current_block = _triplet_role_block(current, "current")
        existing_text = _normalised_spoken_text(
            current_block.get("translated_text")
            or current_block.get("tts_text")
        )
        if existing_text and text.casefold() == existing_text.casefold():
            raise ValueError(
                "Manual override is unchanged. Shorten the spoken text before saving."
            )
        resolution["manual_text"] = text
        resolution["original_character_count"] = len(existing_text)
        resolution["manual_character_count"] = len(text)
        ...
        changed_roles = []
        for role, variant_id in variants.items():
            if not variant_id:
                continue
            block = _triplet_role_block(current, role)
            selected = str(block.get("selected_variant_id") or "").strip()
            if variant_id != selected:
                changed_roles.append(role)
        if not changed_roles:
            raise ValueError(
                "The selected prepared variants are already active. Choose a different variant."
            )
        invalid = [
            value
        ...
        resolution["changed_roles"] = changed_roles
...
def _display_field(block: Mapping[str, Any], key: str) -> str:
    """Return a panel value without erasing legitimate numeric zeroes."""

    value = block.get(key)
    return "" if value is None else str(value)


def _timeline_boundary_card(role: str) -> str:
    labels = {
        "previous": (
            "start of timeline",
            "There is no previous speech block before this collision.",
        ),
        "following": (
            "end of timeline",
            "There is no following speech block after this collision.",
        ),
        "current": (
            "block data unavailable",
            "The collision artifact has no current-block display metadata.",
        ),
    }
    label, explanation = labels.get(
        role,
        ("timeline boundary", "No speech block exists at this position."),
    )
    return f"""
    <section class="block-card boundary-card">
      <h4>{html.escape(role.title())}: {html.escape(label)}</h4>
...
def _block_card(role: str, block: Mapping[str, Any]) -> str:
    ...
    if not block_id:
        return _timeline_boundary_card(role)
    return f"""
    <section class="block-card">
      <h4>{html.escape(role.title())}: {html.escape(block_id)}</h4>
      <p><strong>Speaker:</strong> {html.escape(_display_field(block, 'speaker_id'))}</p>
      <p><strong>Window:</strong> {html.escape(_display_field(block, 'start_ms'))}–{html.escape(_display_field(block, 'end_ms'))} ms</p>
      <p><strong>Source:</strong> {html.escape(_display_field(block, 'source_text'))}</p>
      <p><strong>Selected text:</strong> {html.escape(_display_field(block, 'translated_text'))}</p>
...
def _case_html(job_id: str, case: Mapping[str, Any]) -> str:
    ...
    outcome = (
        case.get("resolution_outcome")
        if isinstance(case.get("resolution_outcome"), Mapping)
        else None
    )
    outcome_html = ""
    if outcome and outcome.get("status") == "did_not_resolve":
        details = []
        for key, label in (
            ("required_expansion_ms", "required adjacent expansion"),
            ("left_silence_capacity_ms", "left silence"),
            ("right_silence_capacity_ms", "right silence"),
            ("required_overlap_ms", "required overlap"),
        ):
            if outcome.get(key) is not None:
                details.append(f"{label}: {html.escape(str(outcome.get(key)))} ms")
        suffix = ("; " + "; ".join(details)) if details else ""
        outcome_html = (
            '<div class="failed"><strong>The previous decision did not fit.</strong> '
            + html.escape(str(outcome.get("reason") or "measured fit still failed"))
            + suffix
            + ". Choose a materially different resolution.</div>"
        )
    cards = "".join(
        _block_card(role, triplet.get(role) or {})
        for role in ("previous", "current", "following")
    )
    available_variant_roles = [
        role
        for role in ("previous", "current", "following")
        if str((triplet.get(role) or {}).get("block_id") or "").strip()
    ]
    allowed_strategies = {
        str(value)
        for value in case.get("allowed_strategies") or ()
        if str(value)
    }
    if not allowed_strategies:
        # Backward-compatible review artifacts predate explicit strategy lists.
        allowed_strategies = {"allow_overlap", "manual_text", "select_variants"}
    has_complete_triplet = all(
        role in available_variant_roles
        for role in ("previous", "current", "following")
    )
    hidden = (
    ...
    overlap_form = ""
    if "allow_overlap" in allowed_strategies:
        overlap_form = f"""
        ...
        """
    variant_scope = "triplet" if has_complete_triplet else "available blocks"
    return f"""
    <article class="collision" id="{html.escape(job_id)}-{html.escape(collision_id)}">
      ...
      <div class="options">
        ...
        <form method="post" action="/resolve">{hidden}<input type="hidden" name="strategy" value="manual_text">
          ...
          <p>The saved wording must be different from the current line. The renderer will remeasure it with Azure.</p>
        ...
        <form method="post" action="/resolve">{hidden}<input type="hidden" name="strategy" value="select_variants">
          <h3>Option 3 — force prepared variants across the {variant_scope}</h3>
          {''.join(f'<label>{role.title()}<select name="{role}_variant_id"><option value="">Keep automatic</option><option>natural</option><option>concise</option><option>compact</option></select></label>' for role in available_variant_roles)}
          <label>Reviewed by<input name="reviewed_by" required></label>
