...
class DirectDubOptions:
    ...
    max_interjection_overlap_ms: int = 1000
    max_interjection_boundary_overlap_ms: int = 750
    ...
    *,
    ...
    for block_index in range(1, len(adjusted) - 1):
        ...
        base_audit: dict[str, Any] = {
            ...
            "max_total_overlap_ms": max_total_overlap_ms,
            "max_boundary_overlap_ms": max_boundary_overlap_ms,
            "text_immutable": True,
            "speaker_order_preserved": True,
        }
        ...
        if safe_slack_ms >= 0:
            ...
            if max_total_overlap_ms <= 0:
                ...
                continue
            if not source_interjection_like:
                audits.append(
                    {
                        **base_audit,
                        "applied": False,
                        "reason": "source_timeline_not_interjection_like",
                        "shortfall_ms": required_overlap_ms,
                        "cross_speaker_overlap": False,
                    }
                )
                continue
            if (
                required_overlap_ms > max_total_overlap_ms
                or required_overlap_ms > 2 * max_boundary_overlap_ms
            ):
                audits.append(
                    {
                        **base_audit,
            ...
            earliest_middle_start_ms = max(
                previous_end_ms - max_boundary_overlap_ms,
                following_start_ms - required_block_ms,
            )
            latest_middle_start_ms = min(
                previous_end_ms,
                following_start_ms + max_boundary_overlap_ms - required_block_ms,
            )
            ...
            overlap_policy = "source_like_sandwiched_interjection"
