...
def _speaker_track_command(
    ...
    filters.append(
        f"{inputs}amix=inputs={len(clips)}:duration=longest:normalize=0:dropout_transition=0,"
        f"alimiter=limit=0.95,apad=whole_len={total_samples},atrim=end_sample={total_samples}[a]"
    )
    command.extend(
        [
            "-filter_complex",
