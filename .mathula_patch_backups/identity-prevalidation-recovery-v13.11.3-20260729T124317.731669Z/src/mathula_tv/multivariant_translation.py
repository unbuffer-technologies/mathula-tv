...
VALIDATION_CONTRACT_VERSION = (
    "mathula-multivariant-validation-v6-undeclared-omission-compression-floor"
)
...
only the risky span, not the surrounding ordinary English. Never infer a
...
def _apply_source_specific_identity_expansion(
    ...
    *,
    ...
    """Expand a generic translated institution noun to its reviewed identity.

    Entity binding may attach a canonical protected identity to a unit whose
    source explicitly names that institution. Claude sometimes translates only
    the generic noun (for example ``iKhomishini``) in all three variants. That is
    not a factual translation error, but it loses the source-specific identity
    and previously caused the whole chunk to be rejected.

    This repair is deliberately narrow and configuration-driven:

    * the source unit must contain a reviewed *specific* source form;
    * the target text must contain a reviewed generic target form;
    * a reviewed specific target form must be derivable by appending a suffix
      to that exact generic form; and
    * no free-form target-language wording is invented.

    The generated suffix comes from ``translation_identity_aliases.json``. The
    duration estimate is increased proportionally and the normal strict
    validator still runs after the repair.
    """
    ...
    for raw_span in protected_spans:
        ...
        generic_source_keys = {value.casefold() for value in generic_source_forms}
        specific_source_forms = [
            value for value in base_forms if value.casefold() not in generic_source_keys
        ]
        if not any(
        ...
        for generic in generic_forms:
            ...
            if not candidates:
                continue
            # Prefer the shortest reviewed expansion. It preserves identity
            # while adding the least timing pressure.
            specific = min(candidates, key=lambda value: (len(value), value.casefold()))
        ...
        for variant in variants:
            ...
            replacement: tuple[str, str, str, re.Match[str]] | None = None
            for generic, suffix, specific in expansions:
                pattern = re.compile(
                    rf"(?iu)(?<![\\w-])([\\w'-]*{re.escape(generic)})(?![\\w-])"
                )
                match = pattern.search(spoken)
                if match is not None:
                    replacement = (generic, suffix, specific, match)
                    break
            if replacement is None:
                continue

            generic, suffix, specific, match = replacement
            ...
            if old_duration > 0:
                ratio = len(expanded) / max(1, len(spoken))
                variant["estimated_duration_ms"] = max(
                    old_duration,
                    int(math.ceil(old_duration * ratio)),
                )
            variant["spoken_text"] = expanded
            repairs.append(
                {
                    "code": "source_specific_identity_expanded_from_generic_target",
                    "required_span": span,
                    "variant_id": str(variant.get("variant_id") or ""),
                    "matched_generic_form": match.group(1),
                    "reviewed_generic_root": generic,
                    "reviewed_specific_form": specific,
                    "replacement_policy": (
                        "append_reviewed_identity_suffix_to_source_grounded_generic_target"
                    ),
                }
            )
    return repairs
...
def _apply_undeclared_omission_compression_floor_fallback(
    ...
    *,
    ...
    for variant_id in VARIANT_IDS:
        ...
        source_words = {
            word.casefold() for word in _identity_words(source_text)
        }
        source_grounded_undeclared: list[str] = []
        for detail in undeclared:
            detail_words = _identity_words(detail)
            semantic_label = (
                bool(detail_words)
                and detail_words[-1].casefold()
                in {"position", "post", "role", "title"}
            )
            anchor_words = [
                word.casefold()
                for word in _identity_words(_compact_optional_detail_anchor(detail))
            ]
            if _contains_protected_span(source_text, detail) or (
                semantic_label
                and len(anchor_words) >= 2
                and all(word in source_words for word in anchor_words)
            ):
                source_grounded_undeclared.append(detail)

        # Do not use a richer variant to launder hallucinated omission metadata.
        # Every undeclared detail must still be grounded in the immutable source.
        if source_grounded_undeclared != undeclared:
            continue
...
def normalize_multivariant_response(
    ...
    *,
    ...
    for raw_unit in raw_units:
        ...
        identity_specific_expansion_repairs = (
            _apply_source_specific_identity_expansion(
                normalized_variants,
