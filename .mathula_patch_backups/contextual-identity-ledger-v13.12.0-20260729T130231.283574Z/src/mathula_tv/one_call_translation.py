...
def _recover_failed_candidate(
    *,
    ...
    """Revalidate a paid failed candidate after a local normalizer upgrade.

    ``complete_structured`` persists its parsed/normalised candidate inside the
    semantic chunk failure artifact when strict validation rejects it.  A later
    deterministic repair must consume that saved candidate before starting a
    new paid provider request.  Recovery is accepted only when the exact chunk
    request hash still matches and the candidate passes the current full schema
    ...
    try:
        ...
        metadata = {
            ...
            "recovery_source": str(failure_path),
            "recovered_at": _utcnow(),
        }
        return normalized, metadata, validation
    except Exception:
        # A missing/truncated legacy candidate is not fatal.  Fall through to a
        # normal provider request while preserving every earlier checkpoint.
        return None
